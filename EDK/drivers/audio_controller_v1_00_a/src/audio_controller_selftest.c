/*****************************************************************************
* Filename:          C:\Projects\EDK_Trial_3/drivers/audio_controller_v1_00_a/src/audio_controller_selftest.c
* Version:           1.00.a
* Description:       Contains a diagnostic self-test function for the audio_controller driver
...
XStatus AUDIO_CONTROLLER_SelfTest(void * baseaddr_p)
{
  ...
  AUDIO_CONTROLLER_mReset(baseaddr);
  ...
  AUDIO_CONTROLLER_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = AUDIO_CONTROLLER_mReadSlaveReg0(baseaddr, 0);
  ...
  AUDIO_CONTROLLER_mWriteSlaveReg1(baseaddr, 0, 2);
  Reg32Value = AUDIO_CONTROLLER_mReadSlaveReg1(baseaddr, 0);
  ...
  AUDIO_CONTROLLER_mWriteSlaveReg2(baseaddr, 0, 3);
  Reg32Value = AUDIO_CONTROLLER_mReadSlaveReg2(baseaddr, 0);
  ...
  AUDIO_CONTROLLER_mWriteSlaveReg3(baseaddr, 0, 4);
  Reg32Value = AUDIO_CONTROLLER_mReadSlaveReg3(baseaddr, 0);
  ...
  Reg32Value = AUDIO_CONTROLLER_mReadReg(baseaddr, AUDIO_CONTROLLER_INTR_IPISR_OFFSET);
  xil_printf("   - IP (user logic) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear IP (user logic) interrupt status register\n\r");
  AUDIO_CONTROLLER_mWriteReg(baseaddr, AUDIO_CONTROLLER_INTR_IPISR_OFFSET, Reg32Value);
  xil_printf("   - enable all possible interrupt(s)\n\r");
  AUDIO_CONTROLLER_EnableInterrupt(baseaddr_p);
